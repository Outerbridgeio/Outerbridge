  ...
  "param2"
	...
	loadMethods = {
		...
		async getSubscribeOperations(nodeData: INodeData): Promise<INodeOptionsValue[]> {
			...
			const filteredOperations = totalOperations.filter((op: IETHOperation) => Object.prototype.hasOwnProperty.call(op.providerNetworks, 'quicknode') && op.providerNetworks['quicknode'].includes(network));
