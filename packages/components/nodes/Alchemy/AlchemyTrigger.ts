  ...
  "param2"
	...
	loadMethods = {
		...
		async getSubscribeOperations(nodeData: INodeData): Promise<INodeOptionsValue[]> {
			...
			const filteredOperations = totalOperations.filter((op: IETHOperation) => Object.prototype.hasOwnProperty.call(op.providerNetworks, 'alchemy') && op.providerNetworks['alchemy'].includes(network));
